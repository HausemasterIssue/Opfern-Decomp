...
public abstract class GUI extends AbstractContainer
{
    ...
    public void handleKeyDown(final int key) {
        ...
        for (Component p = this.focus; p != null; p = (Component)p.getParent()) {
            l.add(0, p);
        }
    ...
    public void handleMouseDown(final int x, final int y) {
        ...
        if (!c.equals(this)) {
            ...
            for (upperParent = c; !this.hasChild(upperParent); upperParent = (Component)upperParent.getParent()) {}
            this.children.remove(upperParent);
            this.children.add(upperParent);
            Collections.sort((List<Object>)this.children, (Comparator<? super Object>)new Comparator<Component>() {
                @Override
    ...
    public void handleMouseRelease(final int x, final int y) {
        ...
        if (button != -1) {
            ...
            for (Component p2 = c; p2 != null; p2 = (Component)p2.getParent()) {
                i.add(0, p2);
            }
            final int ex2 = x;
            final int ey2 = y;
    ...
    public void handleWheel(final int x, final int y, final int step) {
        ...
        final ArrayList<Component> l = new ArrayList<Component>();
        for (Component p = c; p != null; p = (Component)p.getParent()) {
            l.add(0, p);
        }
        final int ex = x;
        final int ey = y;
    ...
    public static int[] calculateRealPosition(final Component c) {
        ...
        for (Component parent = (Component)c.getParent(); parent != null; parent = (Component)parent.getParent()) {
            realX += parent.getX();
            realY += parent.getY();
