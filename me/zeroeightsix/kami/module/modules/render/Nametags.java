...
public class Nametags extends Module
{
    ...
    private void drawNametag(final Entity entityIn) {
        ...
        final String str = entityIn.getName() + (this.health.getValue() ? (" " + this.getHealthColoured(entityIn, Math.round(((EntityLivingBase)entityIn).getHealth() + ((entityIn instanceof EntityPlayer) ? ((EntityPlayer)entityIn).getAbsorptionAmount() : 0.0f)))) : "");
    ...
    private String getHealthColoured(final Entity entity, final int health) {
        final float max = ((EntityLivingBase)entity).getMaxHealth();
        final int result = Math.round(health / max * 100.0f);
        if (result > 75) {
