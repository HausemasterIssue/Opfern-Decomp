...
public class ConstAura extends Module
{
    ...
    public ConstAura() {
        ...
        this.packetReceiveListener = (Listener<PacketEvent.Receive>)new Listener(event -> {
            if (event.getPacket() instanceof SPacketSoundEffect && this.nodesync.getValue()) {
                ...
                if (packet.getCategory() == SoundCategory.BLOCKS && packet.getSound() == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                    for (final Entity e : ConstAura.mc.world.loadedEntityList) {
        ...
        this.packetListener = (Listener<PacketEvent.Send>)new Listener(event -> {
            ...
            if (packet instanceof CPacketPlayer && ConstAura.isSpoofingAngles) {
                ((CPacketPlayer)packet).yaw = (float)ConstAura.yaw;
                ((CPacketPlayer)packet).pitch = (float)ConstAura.pitch;
            }
        }, new Predicate[0]);
    }
    
    public static BlockPos getPlayerPos() {
        return new BlockPos(Math.floor(ConstAura.mc.player.posX), Math.floor(ConstAura.mc.player.posY), Math.floor(ConstAura.mc.player.posZ));
    }
    ...
    static float calculateDamage(final double posX, final double posY, final double posZ, final Entity entity) {
        ...
        if (entity instanceof EntityLivingBase) {
            finald = getBlastReduction((EntityLivingBase)entity, getDamageMultiplied(damage), new Explosion((World)ConstAura.mc.world, (Entity)null, posX, posY, posZ, 6.0f, false, true));
        }
        return (float)finald;
    }
    ...
    private static float getDamageMultiplied(final float damage) {
        final int diff = ConstAura.mc.world.getDifficulty().getId();
        return damage * ((diff == 0) ? 0.0f : ((diff == 2) ? 1.0f : ((diff == 1) ? 0.5f : 1.5f)));
    }
    
    private static void setYawAndPitch(final float yaw1, final float pitch1) {
        ConstAura.yaw = yaw1;
        ConstAura.pitch = pitch1;
        ConstAura.isSpoofingAngles = true;
    }
    
    private static void resetRotation() {
        if (ConstAura.isSpoofingAngles) {
            ConstAura.yaw = ConstAura.mc.player.rotationYaw;
            ConstAura.pitch = ConstAura.mc.player.rotationPitch;
            ConstAura.isSpoofingAngles = false;
        }
    }
    ...
    public static Vec3d getEyesPos() {
        return new Vec3d(ConstAura.mc.player.posX, ConstAura.mc.player.posY + ConstAura.mc.player.getEyeHeight(), ConstAura.mc.player.posZ);
    }
    ...
    public static void glBillboard(final float x, final float y, final float z) {
        ...
        GlStateManager.translate(x - ConstAura.mc.getRenderManager().renderPosX, y - ConstAura.mc.getRenderManager().renderPosY, z - ConstAura.mc.getRenderManager().renderPosZ);
        GlStateManager.glNormal3f(0.0f, 1.0f, 0.0f);
        GlStateManager.rotate(-ConstAura.mc.player.rotationYaw, 0.0f, 1.0f, 0.0f);
        GlStateManager.rotate(ConstAura.mc.player.rotationPitch, (ConstAura.mc.gameSettings.thirdPersonView == 2) ? -1.0f : 1.0f, 0.0f, 0.0f);
    ...
    private void drawString(final BlockPos blockPos, final String str) {
        ...
        glBillboardDistanceScaled(blockPos.x + 0.5f, blockPos.y + 0.5f, blockPos.z + 0.5f, (EntityPlayer)ConstAura.mc.player, 1.5f);
        GlStateManager.disableDepth();
        GlStateManager.translate(-(ConstAura.mc.fontRenderer.getStringWidth(str) / 2.0), 0.0, 0.0);
        ConstAura.mc.fontRenderer.drawStringWithShadow(str, 0.0f, 0.0f, -10027024);
        GlStateManager.popMatrix();
    }
    
    public void onUpdate() {
        if (ConstAura.mc.player == null) {
            return;
        }
        ConstAura.isActive = false;
        final EntityEnderCrystal crystal = (EntityEnderCrystal)ConstAura.mc.world.loadedEntityList.stream().filter(entity -> entity instanceof EntityEnderCrystal).filter(e -> ConstAura.mc.player.getDistance(e) <= this.hitRange.getValue()).map(entity -> entity).min(Comparator.comparing(c -> ConstAura.mc.player.getDistance(c))).orElse(null);
        if (this.explode.getValue() && crystal != null) {
            if (ConstAura.mc.player.canEntityBeSeen((Entity)crystal) || ConstAura.mc.player.getDistance((Entity)crystal) <= this.wallRange.getValue()) {
                ...
                final double selfDamage = calculateDamage(crystal.posX, crystal.posY, crystal.posZ, (Entity)ConstAura.mc.player);
                if (selfDamage < this.maxSelfDmg.getValue()) {
                    if (this.rotate.getValue()) {
                        this.lookAtPacket(crystal.posX, crystal.posY, crystal.posZ, (EntityPlayer)ConstAura.mc.player);
                    }
                    ConstAura.isActive = true;
                    ConstAura.mc.player.swingArm(EnumHand.MAIN_HAND);
                    ConstAura.mc.playerController.attackEntity((EntityPlayer)ConstAura.mc.player, (Entity)crystal);
                    ConstAura.isActive = false;
                }
            }
        }
            ...
            resetRotation();
            if (this.oldSlot != -1) {
                ConstAura.mc.player.inventory.currentItem = this.oldSlot;
                this.oldSlot = -1;
            }
            ConstAura.isActive = false;
        }
        int crystalSlot = (ConstAura.mc.player.getHeldItemMainhand().getItem() == Items.END_CRYSTAL) ? ConstAura.mc.player.inventory.currentItem : -1;
        if (crystalSlot == -1) {
            for (int l = 0; l < 9; ++l) {
                if (ConstAura.mc.player.inventory.getStackInSlot(l).getItem() == Items.END_CRYSTAL) {
                    crystalSlot = l;
                    break;
                }
            }
        ...
        if (ConstAura.mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) {
            offhand = true;
        }
        ...
        if (this.place.getValue()) {
            final List<Entity> entities = (List<Entity>)ConstAura.mc.world.playerEntities.stream().filter(entityPlayer -> !Friends.isFriend(entityPlayer.getName())).sorted((entity1, entity2) -> Float.compare(ConstAura.mc.player.getDistance(entity1), ConstAura.mc.player.getDistance(entity2))).collect(Collectors.toList());
            final List<BlockPos> blocks = this.findCrystalBlocks();
            BlockPos targetBlock = null;
            double lastDamage = 0.0;
            this.target = null;
            for (final Entity entity3 : entities) {
                if (entity3 == ConstAura.mc.player) {
                    continue;
                }
                ...
                for (final BlockPos blockPos : blocks) {
                    ...
                    final double selfDamage2 = calculateDamage(blockPos.x + 0.5, blockPos.y + 1, blockPos.z + 0.5, (Entity)ConstAura.mc.player);
                    final float healthTarget = testTarget.getHealth() + testTarget.getAbsorptionAmount();
                    final float healthSelf = ConstAura.mc.player.getHealth() + ConstAura.mc.player.getAbsorptionAmount();
            ...
            if (!offhand && ConstAura.mc.player.inventory.currentItem != crystalSlot) {
                if (this.autoSwitch.getValue()) {
                    ConstAura.mc.player.inventory.currentItem = crystalSlot;
                    resetRotation();
            ...
            if (this.rotate.getValue()) {
                this.lookAtPacket(targetBlock.x + 0.5, targetBlock.y - 0.5, targetBlock.z + 0.5, (EntityPlayer)ConstAura.mc.player);
            }
            EnumFacing f;
            if (this.rayTrace.getValue()) {
                final RayTraceResult result = ConstAura.mc.world.rayTraceBlocks(new Vec3d(ConstAura.mc.player.posX, ConstAura.mc.player.posY + ConstAura.mc.player.getEyeHeight(), ConstAura.mc.player.posZ), new Vec3d(targetBlock.x + 0.5, targetBlock.y - 0.5, targetBlock.z + 0.5));
            ...
            ConstAura.mc.player.connection.sendPacket((Packet)new CPacketPlayerTryUseItemOnBlock(targetBlock, f, offhand ? EnumHand.OFF_HAND : EnumHand.MAIN_HAND, 0.0f, 0.0f, 0.0f));
        }
        if (this.spoofRotations.getValue() && ConstAura.isSpoofingAngles) {
            if (ConstAura.togglePitch) {
                final EntityPlayerSP player = ConstAura.mc.player;
                player.rotationPitch += (float)4.0E-4;
                ConstAura.togglePitch = false;
            }
            else {
                final EntityPlayerSP player2 = ConstAura.mc.player;
                player2.rotationPitch -= (float)4.0E-4;
                ConstAura.togglePitch = true;
            }
        }
    }
    ...
    private boolean canPlaceCrystal(final BlockPos blockPos) {
        ...
        return (ConstAura.mc.world.getBlockState(blockPos).getBlock() == Blocks.BEDROCK || ConstAura.mc.world.getBlockState(blockPos).getBlock() == Blocks.OBSIDIAN) && ConstAura.mc.world.getBlockState(boost).getBlock() == Blocks.AIR && ConstAura.mc.world.getBlockState(boost2).getBlock() == Blocks.AIR && ConstAura.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost)).isEmpty() && ConstAura.mc.world.getEntitiesWithinAABB((Class)Entity.class, new AxisAlignedBB(boost2)).isEmpty();
    }
    ...
    public void onEnable() {
        ...
        if (this.announceUsage.getValue()) {
            Command.sendChatMessage("[ConstAura] " + ChatFormatting.GREEN.toString() + "Enabled!");
        }
    }
    ...
    public void onDisable() {
        ...
        resetRotation();
        if (this.announceUsage.getValue()) {
            Command.sendChatMessage("[ConstAura] " + ChatFormatting.RED.toString() + "Disabled!");
        }
    }
